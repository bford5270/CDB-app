...
function buildOdcSystemPrompt(today) {
  return [
    ...
    '',
    'DATE FORMAT: Promotion History dates are 6-digit DDMMYY → convert to YYYY-MM-DD.',
    '  Parse as: first 2 digits = DD (day), middle 2 = MM (month), last 2 = YY (year).',
    '  YY is ALWAYS a 2000s year: 00→2000, 12→2012, 18→2018, 24→2024. NEVER interpret YY as 1900s.',
    '  Examples: "010924" → day=01, month=09, year=24 → 2024-09-01  (Sep 01, 2024)',
    '            "010918" → day=01, month=09, year=18 → 2018-09-01  (Sep 01, 2018)',
    '            "210512" → day=21, month=05, year=12 → 2012-05-21  (May 21, 2012)',
    '            "011122" → day=01, month=11, year=22 → 2022-11-01  (Nov 01, 2022)',
    'Valid DOR years: 2000–2032. Discard any entry whose converted year falls outside this range.',
    'CRITICAL MISTAKE TO AVOID: "010924" is NOT month=01 (January 9). The FIRST two digits are the DAY.',
    '  day=01, month=09, year=24 → Sep 01, 2024. Day-first (DDMMYY), not month-first.',
    '  If your CDR date shows January when the officer says September, you have swapped DD and MM — re-parse.',
    '',
    ...
    '  14 = Pediatrics                |  15 = Psychiatry          |  16 = Internal Medicine',
    '  17 = General/Family Practice   |  18 = Emergency Medicine  |  19 = Anesthesiology',
    ...
    'Still include future-dated entries in rankHistory so the promotion timeline is complete.',
    '',
    '=== OTHER ODC FIELDS ===',
...
function buildPsrSystemPrompt(odcResult) {
  ...
  return [
    ...
    '',
    'DATE FORMAT FOR startDate/endDate: PSR period dates are 6-digit DDMMYY → convert to YYYY-MM-DD.',
    '  Parse as: first 2 digits = DD (day), middle 2 = MM (month), last 2 = YY (year, always 2000s).',
    '  Examples: "020724" → day=02, month=07, year=24 → 2024-07-02  (Jul 02, 2024)',
    '            "300425" → day=30, month=04, year=25 → 2025-04-30  (Apr 30, 2025)',
    '            "011122" → day=01, month=11, year=22 → 2022-11-01  (Nov 01, 2022)',
    '            "120612" → day=12, month=06, year=12 → 2012-06-12  (Jun 12, 2012)',
    '  IMPORTANT: The FIRST two digits are the day, not the month. Day=30 is valid; month=30 is not.',
    ...
    '',
    'From PSR: each FITREP has pay grade, from/to dates, station, reporting senior name,',
    '5 trait scores (1-5), individual average, RS cumulative average (RSCA), RS count,',
    'promotion rec (EP/MP/P/PR/SP/NOB), report type (RG/CC/AT/TR/NOB).',
    '',
    '=== INDIVIDUAL AVERAGE vs RS AVERAGE (RSCA) ===',
    'These are TWO SEPARATE numbers in EACH FITREP row. Copy them EXACTLY as printed.',
    'individualAverage: this officer\'s own average score for the period (typically 3.5–5.0).',
    'rsAverage: the Reporting Senior\'s Cumulative Average (RSCA) — the mean of ALL officers this RS',
    '  has ever rated (their running average). Look for column labeled "R/S CUM" or "RSCA".',
    '  It appears as a separate number, usually AFTER individualAverage in the row.',
    'DO NOT swap these. DO NOT calculate either — read them verbatim from the document.',
    'If a value is not present, use 0. Never guess or infer either average.',
    '',
    '=== PROMOTION RECOMMENDATION ===',
    'Each FITREP row has TWO separate columns near the right side of the table:',
    '  1. PROMOTION REC grid — 5 checkbox columns: SP | PR | P | MP | EP (left=worst, right=best)',
    '  2. PRT column — Physical Readiness Test result: P=passed, N=not performed, B=not observed, etc.',
    '',
    '!!! CRITICAL: PRT IS NOT THE PROMOTION RECOMMENDATION !!!',
    'The PRT column records the officer\'s physical fitness test result. It has NOTHING to do with',
    'promotion recommendation. NEVER use the PRT column to determine promotionRec.',
    'A PRT value of "P" means the officer passed their physical fitness test.',
    'A PRT value of "N" means the officer was not observed for the fitness test.',
    'Do NOT map PRT values (P, N, B, PP, PN) to promotion recommendations.',
    '',
    'THE ONLY WAY to determine promotionRec is the X mark in the PROMOTION REC grid.',
    'The grid has 5 columns labeled SP, PR, P, MP, EP (sometimes in reverse order EP, MP, P, PR, SP).',
    'Find which column label is adjacent to the X mark — that label is the promotionRec.',
    '  Text "EP X" or "X EP" or "EP: X" → promotionRec = "EP"',
    '  Text "MP X" or "X MP" or "MP: X" → promotionRec = "MP"',
    '  Text "P X" or "X P" or "P: X" → promotionRec = "P"',
    '  Text "PR X" or "X PR" or "PR: X" → promotionRec = "PR"',
    '  Text "SP X" or "X SP" or "SP: X" → promotionRec = "SP"',
    '  No X in any column → "NOB"',
    '',
    'The number adjacent to each label is the count of OTHER officers with that rec — NOT this officer\'s.',
    'Example: "SP 0 PR 0 P 12 MP X 8 EP 3" → the X is next to MP → promotionRec = "MP"',
    'Example: "EP X 3 MP 8 P 12 PR 0 SP 0" → the X is next to EP → promotionRec = "EP"',
    '',
