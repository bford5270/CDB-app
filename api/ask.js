  ...
  try {
    ...
    if (action === 'parse-documents') {
      ...
      const parseSystemPrompt = [
        ...
        '',
        '=== AQD EXTRACTION ===',
        'GOAL: Reflect what the documents actually say. Do NOT validate or filter against any reference list.',
        '',
        'SOURCE 1 — ODC Block 72: Look for the section explicitly labeled "Additional Qualification Designators", "AQD", or "Block 72".',
        '  Extract each code you find there exactly as written.',
        '',
        'SOURCE 2 — OSR Special Qualifications: Look for a section labeled "Special Qualifications", "Additional Qualifications",',
        '  "AQDs", or similar. Extract any AQD codes listed there. OSR may spell out names (e.g. "67A Executive Medicine") —',
        '  extract the code portion (e.g. "67A").',
        '',
        'MERGE: Combine codes from both sources. Deduplicate (case-insensitive). If a code appears in both, include it once.',
        '',
        'EXCLUDE only these specific noise patterns (not AQDs):',
        '  - Two-letter clearance codes appearing near "clearance": SS, TT, VV, TS',
        '  - Rank abbreviations: ENS, LTJG, LT, LCDR, CDR, CAPT',
        '  - 4-digit designator codes (2100–2399)',
        '  - Report type codes: RG, CC, AT, TR',
        '  - Specialty codes that END in K, J, or T (e.g. "16Q0K", "2100J") — the AQD block is separate from the specialty block',
        'If no AQD section is found in either document, return [].',
        '',
        '=== BOARD CERTIFICATION ===',
        ...
        '',
        'CRITICAL: The PSR has TWO ways to identify promotion rec. Use BOTH — prefer Method A.',
        '',
        'METHOD A — "PRT" ABBREVIATION COLUMN (most reliable):',
        'The PSR has a short "PRT" column near the right side of each FITREP row with a 1-2 char code:',
        '  "N"  = NOB (Not Observed)',
        '  "B"  = NOB (Not Observed) — appears on CC/admin reports with IND avg 0.00',
        '  "P"  = Promotable (P)',
        '  "PP" = Early Promote (EP) — the letter "E" often OCRs as "P" in Navy PDFs; "PP" means EP, NOT P',
        '  "MP" = Must Promote (MP)',
        '  "SP" = Select Promotable (SP) — lowest rec',
        '  "PR" = Progressing (PR)',
        'IMPORTANT: "PP" is a TWO-character code meaning EP. Do NOT read it as a single "P". If you see "PP", set promotionRec = "EP".',
        '',
        'METHOD B — 5-COLUMN MARKER PATTERN (backup):',
        'The PSR has 5 adjacent marker columns in left-to-right order: SP | PR | P | MP | EP.',
        '(Note: SP is leftmost/lowest, EP is rightmost/highest — opposite of what you might expect.)',
        'Exactly ONE column is marked; the rest are blank/zero.',
        '  Position 1 (leftmost) marked = SP',
        '  Position 2 marked = PR',
        '  Position 3 marked = P',
        '  Position 4 marked = MP',
        '  Position 5 (rightmost) marked = EP',
        'If the marker pattern is ambiguous or absent, set promotionRec to null — do NOT default to "P".',
        '',
        'STEP 3 — LOOK FOR SUMMARY TOTALS: The PSR may have a summary row like "EP 3  MP 5  P 2".',
        '  Use these totals to set earlyPromotes, mustPromotes, promotables counts.',
        '',
        'NOB (Not Observed) reports: PRT shows "N" or "B"; reportType will be AT, TR, CC, or NOB.',
