...
function buildPsrSystemPrompt(odcResult) {
  ...
  return [
    ...
    '  endDate   → END DATE on LINE 2 (MMDDYY format)',
    '  station   → station part1 from LINE 1 + part2 from LINE 2 combined',
    ...
    'The grid columns are always in order: SP | PR | P | MP | EP (worst → best, left to right).',
    'Count which column the X is in. That column label is the promotionRec.',
    '',
    'REAL EXAMPLES from a CDR\'s PSR (LINE 1 endings):',
    '  "0 0 4 2 0  3.33  307  X p  RG" → X is in the P column → promotionRec = "P"',
    '  "0 0 1 3 2  4.17  321  X p  RG" → X is in the P column → promotionRec = "P"',
    '  "0 0 2 1 3  4.17    0  X N  RG" → X is in the EP column → promotionRec = "EP"',
    '',
